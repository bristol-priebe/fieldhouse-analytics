...
datatable(ratings, container = sketch, extensions = "FixedColumns", rownames = FALSE, class = "cell-border stripe", options = list(columnDefs = list(list(className = "dt-center", targets = c(0,2:16)), list(orderable = FALSE, targets = c(4,9:16))), pageLength = 385, scrollX = TRUE, scrollY = '700px', fixedColumns = list(leftColumns = 2))) %>% formatRound("CurrentRating", digits = 2) %>% formatRound("ExpWinPct", digits = 3) %>% formatRound("WinsOverExp", digits = 1)
```
